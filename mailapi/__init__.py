...
class MailAPI:
    ...
    async def isValidLink(self, link: str) -> bool:
        ...
        async with aiohttp.ClientSession() as s, s.post(
            "https://mublic.cloud.mail.ru/api/m3/list",
    ...
    async def getList(
        self, link: str, limit: int = 1000, offset: int = 0
    ) -> Union[Models.Files, Models.File, Models.NoFiles]:
        if not self.url.match(link):
            raise ValueError(f'Url/path "{link}" is invalid!')
        path = self.url.findall(link)[0][-1]
        async with aiohttp.ClientSession() as s, s.post(
        ...
        if "objects" not in obj:
            if "name" in obj:
                async with aiohttp.ClientSession() as s, s.post(
                    "https://mublic.cloud.mail.ru/api/m3/get",
                    headers=self.headers,
                    json={"path": path},
                ) as response:
